...
Marqo官方声称FashionSigLIP比FashionCLIP高57% MRR/Recall。我们之前的PDF提取图实验显示FashionCLIP更优（R@5 +7.3%），但**真实B2B商品图实验已验证Marqo的结论**：

#### 真实B2B商品图验证（Image→Image，49 queries / 1319 gallery）

| 指标 | FashionSigLIP | FashionCLIP | SigLIP领先 |
|------|:---:|:---:|:---:|
| R@1 | **0.5714** | 0.4898 | **+16.7%** |
| R@5 | **0.5143** | 0.4367 | **+17.8%** |
| R@10 | **0.4306** | 0.3571 | **+20.6%** |
| R@20 | **0.3673** | 0.3102 | **+18.4%** |
| NDCG@5 | **0.5283** | 0.4463 | **+18.4%** |
| NDCG@10 | **0.4661** | 0.3875 | **+20.3%** |

> 数据集：从`相似商品汇总_50个.docx`提取的真实B2B闪屏商品图，49个query、1319个gallery商品，ground truth为多模型相似商品结果的并集。

**FashionSigLIP在Image→Image场景下同样全面领先**，印证了Marqo官方的结论。之前PDF图上FashionCLIP领先是因为数据域偏移，而非模型本身的能力差异。

这也证明了GCL训练不仅提升了text→image能力，image embedding本身也编码了更丰富的品类/风格信息。

### 9.6.1 三方扩展评测：加入jina-clip-v2

在真实B2B商品图上扩展评测，加入jina-clip-v2（ONNX推理，1024维）：

| 指标 | FashionSigLIP | FashionCLIP | jina-clip-v2 | 最优 |
|------|:---:|:---:|:---:|:---:|
| R@1 | **0.5714** | 0.4898 | 0.4490 | SigLIP |
| R@5 | **0.5143** | 0.4367 | 0.3714 | SigLIP |
| R@10 | **0.4306** | 0.3571 | 0.3388 | SigLIP |
| NDCG@5 | **0.5283** | 0.4463 | 0.3903 | SigLIP |
| NDCG@10 | **0.4661** | 0.3875 | 0.3612 | SigLIP |
| 推理时间(CPU) | 342s | 131s | **9928s** | — |
| Embedding维度 | 768 | 512 | 1024 | — |

**jina-clip-v2在时尚场景表现最差**，R@5仅0.3714，且CPU推理耗时近3小时（512×512 ONNX），性价比极低。通用多语言模型在垂直时尚领域的细粒度匹配能力明显不如领域微调模型。

#### 分组观察（docx标签，仅供参考，非评估标准）

docx将50个query分为"效果更好"(15个)、"效果相近"(22个)、"效果欠缺"(13个)三组，这是人工主观判断，仅作探索性参考：

| 分组 | SigLIP R@5 | FCLIP R@5 | jina R@5 | 观察要点 |
|------|:---:|:---:|:---:|------|
| 效果更好 | — | — | — | 数据未单独解析 |
| 效果相近 | 0.5111 | 0.4667 | 0.2444 | SigLIP领先，jina明显弱 |
| 效果欠缺 | 0.5077 | **0.6000** | 0.4000 | FCLIP反超SigLIP |

> ⚠️ "效果欠缺"组FCLIP表现更好，暗示在视觉差异大的query上，B/32的粗粒度特征可能比B/16更鲁棒。但样本量小（13 queries），需谨慎解读。

### 9.6.2 结果级融合实验

在真实B2B商品图上测试结果级融合（min-max归一化后加权）：

#### 2-way融合

| 组合 | 最优α | R@5 | NDCG@5 | vs SigLIP单模型 |
|------|:---:|:---:|:---:|:---:|
| **SigLIP+FCLIP** | **0.6** | **0.5510** | **0.5769** | **R@5 +7.2%, NDCG@5 +9.2%** |
| SigLIP+jina | 0.7 | 0.5184 | 0.5343 | R@5 +0.8% |
| FCLIP+jina | 0.6 | 0.4898 | 0.5166 | 不如SigLIP单模型 |

#### 3-way融合

| 权重(SigLIP/FCLIP/jina) | R@5 | R@10 | NDCG@5 | NDCG@10 |
|:---:|:---:|:---:|:---:|:---:|
| **0.6/0.3/0.1** | **0.5551** | 0.4714 | **0.5777** | 0.5118 |
| 0.6/0.2/0.2 | 0.5551 | 0.4694 | 0.5727 | 0.5079 |
| 0.4/0.4/0.2 | 0.5551 | 0.4653 | 0.5764 | 0.5074 |
| 0.5/0.3/0.2 | 0.5510 | 0.4633 | 0.5731 | 0.5052 |

**融合结论**：
- **SigLIP+FCLIP 2-way融合（α=0.6）已接近3-way最优**，R@5从0.514→0.551（+7.2%），NDCG@5从0.528→0.577（+9.2%）
- 加入jina-clip-v2几乎没有额外收益，3-way最优仅比2-way高0.4% R@5
- 融合生效的原因：SigLIP擅长细粒度匹配，FCLIP擅长粗粒度/视觉差异大的case，两者互补
- **生产推荐**：SigLIP+FCLIP双路检索，α=0.6/0.4，不需要引入jina-clip-v2

### 9.7 选型建议

| 场景 | 推荐 | 理由 |
|------|------|------|
| **Image→Image检索** | **FashionSigLIP** | 真实B2B图R@5 +18%全面领先 |
| **Text→Image检索** | **FashionSigLIP** | 官方评测全面领先 |
| **有品类标签的检索** | **FashionSigLIP** | Category→Product官方P@1=0.758 |
| **最优召回率** | **SigLIP+FCLIP融合** | R@5=0.551, NDCG@5=0.577，比单模型+9.2% |
| **低质量/压缩图检索** | FashionCLIP | B/32对图片退化更鲁棒 |
| **轻量部署/边缘设备** | FashionCLIP | 512维/131s vs 768维/342s |
| **生产环境** | **SigLIP+FCLIP双路** | α=0.6/0.4融合，互补效果最佳 |
| ~~通用多语言模型~~ | ~~jina-clip-v2~~ | 时尚场景最差+CPU推理9928s，不推荐 |
...
1. **FashionSigLIP是最优单模型** — 真实B2B商品图Image→Image全面领先（R@5 +18%），PDF图上落后是数据域偏移
2. **SigLIP+FCLIP双路融合是最优方案** — R@5=0.551(+7.2%), NDCG@5=0.577(+9.2%)，α=0.6/0.4
3. **jina-clip-v2在时尚场景不可用** — R@5仅0.3714，CPU推理9928s，通用模型不具备细粒度时尚匹配能力
4. **数据集质量是第一要素** — PDF图 vs 真实图的差距远大于模型间的差距
5. **GCL训练让image embedding也更强** — 不只是text→image受益，image→image同样显著提升
6. **结果级融合比Embedding级融合更可靠** — Slerp在强模型上几乎无效，结果级加权在真实图上稳定+7~9%
7. **SigLIP和FCLIP互补** — SigLIP擅长细粒度，FCLIP擅长视觉差异大的case，融合生效
