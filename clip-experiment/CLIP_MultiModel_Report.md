...
| 场景 | 推荐 | 理由 |
|------|------|------|
| **Image→Image检索（真实商品图）** | **FashionSigLIP** | R@5 +18%、NDCG@5 +18%，全面领先 |
...
| **轻量部署/边缘设备** | FashionCLIP | 512维/12img/s vs 768维/4img/s |
...
1. **FashionSigLIP是最优模型** — 真实B2B商品图Image→Image全面领先（R@5 +18%），PDF图上落后是数据域偏移
2. **纯image检索在时尚场景严重不足** — PDF图上所有模型R@5<0.14，但真实图上R@5可达0.51
3. **数据集质量是第一要素** — PDF图 vs 真实图的差距远大于模型间的差距
4. **GCL训练让image embedding也更强** — 不只是text→image受益，image→image同样显著提升
5. **Embedding级Slerp融合在弱基线模型上有效** — FashionSigLIP +256%提升，但强模型提升小
6. **Auto-caption在当前条件下无效** — BLIP太粗糙，Zero-shot是自循环
7. **结果级融合是正确路径** — `score = α·img_sim + (1-α)·text_sim`
