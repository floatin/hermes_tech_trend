...
### 9.3 效果差的根因：不是图片质量，是Embedding空间坍缩
...
|------|--------------|-------------|
| True Positive | 2332 | 1624 |
| False Positive | **18345** | 6030 |
| Precision | 11.3% | 21.2% |
| FP Rate | 43.6% | 14.3% |

SigLIP找到的同类对确实更多（TP 2332 vs 1624），但付出的代价是**3倍的虚假阳性**——它把43.6%的跨品类对误判为相似。

#### 9.3.2 Precision-Recall全面对比

| 阈值 | SigLIP P | SigLIP R | SigLIP F1 | FCLIP P | FCLIP R | FCLIP F1 |
|------|---------|---------|----------|---------|---------|----------|
| 0.50 | 0.063 | 0.999 | 0.119 | 0.074 | 0.979 | 0.138 |
| 0.60 | 0.073 | 0.972 | 0.136 | 0.106 | 0.879 | 0.189 |
| 0.70 | 0.113 | 0.845 | 0.199 | 0.212 | 0.588 | **0.312** |
| 0.75 | 0.164 | 0.670 | 0.264 | 0.320 | 0.365 | **0.341** |
| 0.80 | 0.253 | 0.419 | **0.316** | 0.479 | 0.168 | 0.249 |

FashionCLIP在低阈值区间(0.5~0.75)F1全面领先；SigLIP只在0.8+高阈值才反超——但此时两者召回率都已很低。

#### 9.3.3 Embedding空间坍缩：B/16 + GCL的副作用

| 因素 | SigLIP | FCLIP |
|------|--------|-------|
| 基座架构 | ViT-B-16 (196 patches) | ViT-B/32 (49 patches) |
| 训练方法 | GCL (细粒度标签) | 标准CLIP对比学习 |
| 平均跨图相似度 | **0.685** | 0.595 |
| 跨模型结构对齐(Spearman ρ) | vs OpenAI=0.682 | vs OpenAI=0.747 |

**三个叠加因素导致SigLIP的embedding空间坍缩**：

1. **ViT-B/16更敏感**：196个patch vs 49个，更多细粒度特征被编码。在高质量图片上这是优势，但当品类间的细节差异（纹理、材质）被PDF提取损失后，这些额外patch只增加了噪声
2. **GCL训练的副作用**：引入category/style/color/material等细粒度标签，模型学会了"红连衣裙和蓝连衣裙都是dress"（类内拉近），但也学会了"红连衣裙和红T恤有共同颜色特征"（跨品类也拉近）
3. **Sigmoid Loss不强制全局区分**：对比每个pair独立计算，不像Softmax loss那样在batch内做全局排序，导致embedding更"拥挤"

**核心洞察**：不是"图片质量差导致SigLIP效果差"，而是"SigLIP的架构+训练方式让embedding空间过于拥挤，在低质量图片上这个问题被放大了"。FashionCLIP更"粗糙"（更少patch、更简单的训练），反而在这个数据集上更鲁棒。

#### 9.3.4 Per-Category胜率验证

| 统计 | SigLIP胜 | FCLIP胜 |
|------|---------|---------|
| 类别数 | 6/21 | 15/21 |
| SigLIP胜的类别平均类内相似度 | 0.753 | — |
| FCLIP胜的类别平均类内相似度 | — | 0.725 |

SigLIP只在"视觉高度一致"的类别上赢（如p1,p16,p24——同PDF页面的图片视觉风格高度统一），FCLIP在大多数类别上都胜出。这进一步说明SigLIP依赖图片的视觉一致性，而非品类语义。
